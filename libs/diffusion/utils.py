...
def setup_log_directory(config):
    ...
    if os.path.isdir(config.root_log_dir):
        ...
        folder_list = os.listdir(config.root_log_dir)
        # remove .ini files
        folder_list = [folder for folder in folder_list if not folder.endswith(".ini")]
        folder_numbers = [int(folder.replace("version_", "")) for folder in folder_list]
