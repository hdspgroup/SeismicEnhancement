...
class ContextEmbeddings(nn.Module):
    def __init__(self, time_emb_dims=128, time_emb_dims_exp=512):
        super().__init__()
        ...
        ts = torch.arange(5, dtype=torch.float32) 
...
class ResnetBlock(nn.Module):
    def __init__(self, *, in_channels, out_channels, dropout_rate=0.1, time_emb_dims=512, apply_attention=False, context_embd_dims=None):
        ...
        if context_embd_dims is not None:
            self.dense_context = nn.Linear(in_features=context_embd_dims, out_features=self.out_channels)
    ...
    def forward(self, x, t, c=None):
        ...
        if c is not None:
            h *= self.dense_context(self.act_fn(c))[:, :, None, None]
