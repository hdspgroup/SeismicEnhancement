...
from .layers import ( SinusoidalPositionEmbeddings,
                     ContextEmbeddings,
                     ResnetBlock,
                     DownSample,
                     UpSample )
...
class ContextUNet(nn.Module):
    def __init__(
        ...
        context_emb_dims_exp = base_channels * context_multiple
        self.context_embeddings = ContextEmbeddings(time_emb_dims=base_channels, time_emb_dims_exp=context_emb_dims_exp)
        ...
        for level in range(num_resolutions):
            ...
            for _ in range(num_res_blocks):
                ...
                block = ResnetBlock(
                    ...
                    time_emb_dims=time_emb_dims_exp,
                    apply_attention=apply_attention[level],
                    context_embd_dims=context_emb_dims_exp,                    
                )
                self.encoder_blocks.append(block)
    ...
    def forward(self, x, t, c=None):
        ...
        time_emb = self.time_embeddings(t)
        context_emb = self.context_embeddings(c)

        h = self.first(x)
        outs = [h]

        for layer in self.encoder_blocks:
            h = layer(h, time_emb, context_emb)
            outs.append(h)
